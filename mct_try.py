...
class VanilaMCTS:
    ...
    def expand(self, selected_action_id):
        winner, _ = self.is_terminal_state(self.state_tree[selected_action_id]["state"])
        if winner:
            return selected_action_id
    ...
    def simulate_play(self, child_action_id):
        ...
        winner, reward = self.is_terminal_state(self.state_tree[child_action_id]["state"])
        if winner:
            return reward
        ...
        while not is_game_end:
            ...
            action_position, _ = available_actions[random_action_choice]
            if state["player"] == "x":
                state["player"] = "o"
            ...
            winner, reward = self.is_terminal_state(state["state"])
            if winner:
                return reward

    def backprop_stats(self, child_action_id, reward):
        while True:
    ...
    def solve(self):
        ...
        for _ in range(self.num_itearions):
            ...
            if play_depth >= self.max_depth:
                break
            reward = self.simulate_play(child_action_id)
            self.backprop_stats(child_action_id, reward)
